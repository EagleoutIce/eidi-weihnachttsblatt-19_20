...
% \attachfilesetup{author=Florian Sihler}

\begin{document}
    
\maketitle

% too great to miss: https://tex.stackexchange.com/questions/387047/the-duck-pond-showcase-of-tikz-drawn-animals-ducks
\begin{tikzpicture}[overlay,remember picture]
    \node at ($(current page.north east)+(-3.75cm,-7.25cm)$) {\begin{tikzpicture}[scale=0.75]
        \duck[santa=red!80!black, jacket=red!80!gray, tshirt=white,water];
...
\begin{solution}
    ...
    {\upshape\ijava[linerange={32-40},firstnumber=32]{easy/A21_Schaltjahrpruefer.java}}
...
% TODO TODO TODO: MAKE GIT WITH ALL OF THIS INCLUDING TEX-CODE AND STUFF
...
\begin{minipage}{0.45\linewidth}
    \centering\begin{tikzpicture}[scale=0.75, every node/.style={transform shape}]
        ...
        \foreach \i/\xi in {2/2,6/3,8/4}{
            \def\sX##1{\ifnum\i=##1 \cellcolor{sob@col@uulm@cs!15!white}{O}\else##1\fi}
            \def\dX##1{\ifnum\i=##1 \bfseries O\else O\fi}
            \def\dsX##1{\ifnum\i=##1 \cellcolor{sob@col@uulm@akzent!25!white}{\bfseries O}\else##1\fi}
...
\begin{solution}
    ...
    In dieser Aufgabe basteln wir uns erst einmal eine Liste. An sich werden wir uns eine Klasse konstruieren, die ein Array für uns verwaltet, welches dynamisch mit unseren Bedürfnissen wachsen und Schrumpfen kann. Eine ähnliche (naja, das ist schon anmaßend, sagen wir, im Kerngedanken) Implementation ist bereits durch die Klasse \bjava{ArrayList} gegeben, die wir hier allerdings nicht verwenden. Ich werde hier nicht genau auf die Implementation eingehen, da die Funktionsweise der Methoden bereits Im Tutorium zu Übungsblatt \(7\) von mir im Rahmen der Präsenzaufgabe besprochen wurde (Siehe hier: \attachDocumentText{../Studenten-Share/folien8_ub_7_references_equals.pdf}{\pc{} \detokenize{folien8_ub_7_references_equals.pdf}}). Hier also nur die Sammlung der Methoden die wir im Folgenden verwenden können.
    \bgroup
